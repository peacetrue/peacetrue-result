...
不传入参数，触发 `MissingServletRequestParameterException`，返回数据如下：
...
触发 `MissingPathVariableException`，返回数据如下：
...
传入参数 `input=a`，触发 `MethodArgumentTypeMismatchException`，返回数据如下：
...
传入违反上述所有规则的参数，触发 `BindException`，返回数据如下：

.响应结果
[source%nowrap,json]
----
include::example$peacetrue-result-exception-support/src/test/resources/com/github/peacetrue/result/exception/beanInvalid.json[]
----
